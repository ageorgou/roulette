function [result,success] = samplePosteriorNew_toggle(obs,A,states)
...
fullInit = [obs(1,2:end) 0 1];
initState = encodeStates(fullInit,states);
%for n = 2:nSamples
    ...
    while ~success
        deb('Running FFBS');
        %[succ,probV,newV] = FFBS_toggle(B,W,obs(1,2:end),obs(2:end,:),states,[]);
        [succ,probV,newV] = FFBS_toggle(B,W,fullInit,obs(2:end,:),states,[]);
        if ~succ
            success = false;
            result = [];
